 ...
 */
public class CommonInfoDialog extends Toast {
	/**
	...
	public CommonInfoDialog(Context context, int infoType, int mesId, int duration)
	{
		super(context);
		mContext=context;
		mDuration = duration;
	...
	public static CommonInfoDialog makeDialog(Context context, int infoType, int mesId, int duration)
	{
		CommonInfoDialog result = new CommonInfoDialog(context, infoType, mesId, duration);
	...
	private void initView() {
		...
		View view = layout.inflate(R.layout.common_info_dialog,null);
		
		tv = (TextView) (view.findViewById(R.id.common_info_hintmessage));
		tv.setText(getMessage());

		info_layout = (LinearLayout)view.findViewById(R.id.common_info_layout);
		
		hintIcon=(ImageView)(view.findViewById(R.id.common_info_hinticon));
		switch(mInfoType)
		{
		case TYPE_OK:
			setHintIcon(0);
		...
		this.setGravity(Gravity.BOTTOM|Gravity.CENTER_HORIZONTAL, 0, 30);
	}
		...
		TimerTask task = new TimerTask() {
			public void run() {
				...
				else
				{
					...
					timer.cancel();
					Log.v("CHLauncher", "mToast.cancel();");
				}
			}
		};
